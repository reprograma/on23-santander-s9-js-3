...
preencherPagina(json) //passa o parâmetro json ao chamar a função 

/*function preencherPagina(series){
  series.map((serie) => {
    //esse bloco faz o card
    const card = document.createElement("section")
    card.classList.add('cards')
    container.appendChild(card) 

    const cover = document.createElement("img") // criei uma imagem <img>
    ...
    genders.innerHTML = `Gêneros: <span class="dynamic-text">${serie.diretor}</span>`
    card.appendChild(genders)

    const cast = document.createElement("p")
    cast.classList.add('contents')
    cast.innerHTML = `Gêneros: <span class="dynamic-text">${serie.diretor}</span>`
    card.appendChild(cast)

    const instagram = document.createElement("p")
    instagram.classList.add('contents')
    instagram.innerHTML = 
    card.appendChild(genders)
